...
it('should not click iframe overlaying the target', async ({ page, server }) => {
  ...
  expect(error.message).toContain(`<iframe srcdoc="<body onclick='window.top._clicked=2' style='background-color:red;height:40px;'></iframe> from <div>…</div> subtree intercepts pointer events`);
});
